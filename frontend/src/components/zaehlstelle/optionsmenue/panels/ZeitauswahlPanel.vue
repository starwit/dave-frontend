    ...
    <v-expansion-panel-text class="mt-1">
      ...
      <v-row no-gutters>
        ...
        >
          <v-select
            v-if="isZeitauswahlZeitraum"
            v-model="chosenOptionsCopy.holidayOptions"
            label="Ferien"
...
const isZeitauswahlZeitraum = computed(() => {
  return chosenOptionsCopy.value.zeitauswahl === Zeitauswahl.ZEITRAUM;
});
...
function zeitauswahlChanged() {
  ...
  if (isZeitauswahlZeitraum.value) {
    ...
    if (!chosenOptionsCopy.value.tagesTyp && tt) {
      chosenOptionsCopy.value.tagesTyp = tt;
    }
    const ho = holidayOptionsInfo.get(holidayOptionsValues.value[0].value)?.value;
    if (!chosenOptionsCopy.value.holidayOptions && ho) {
      chosenOptionsCopy.value.holidayOptions = ho;
    }
  } 
}
